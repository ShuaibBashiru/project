    ...
    try:
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT * FROM user_widgets WHERE record_status=%s"
                                    " ORDER BY time_modified, date_modified, status_id ", [1, ])
...
def widget_list_filter(request):
    try:
...
def widget_list_search(request):
    try:
        search = request.GET['search']
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT * FROM user_widgets WHERE "
                                    "record_status=%s AND widgetName=%s OR widgetTitle=%s "
                                    "ORDER BY time_modified, date_modified, "
                                    "status_id", [1, search, search])
            row = dictfetchall(cursor)
            if counter > 0:
                feedback = {
                   'status': 'success',
...
def download_widget(request):
    try:
        with connection.cursor() as cursor:
            ...
            filename = 'Widget_list_{}_{}.csv'.format(date_modified, time_modified)
