...
def listdata():
    ...
    if request.method=='POST':
        ...
        if tablename=='leveldata':
            ...
            res=cur.execute("SELECT * FROM image_tbl i INNER JOIN category_tbl c on c.id=i.classid ORDER BY others ASC")
        if res > 0:
            row_headers=[x[0] for x in cur.description]
            rv = cur.fetchall()
            for result in rv:
...
def bulkimages():
    ...
    for file in getfiles:
        ...
        res=cur.execute("INSERT INTO image_tbl(classid, filename, height, width, vresolution, hresolution, dimensions, others, dateCreated) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)", (classid, str(randomnum)+file.filename, height, width, vresolution, hresolution, dimensions, size, dateCreated))
        if res:
            mysql.connection.commit()
    return 'created'
