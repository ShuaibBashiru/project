...
def train_data(request):
    ...
    data = pd.read_csv('frontend/src/assets/uploaded/coviddataset.csv')
    print(data.shape)
    plt.figure(figsize=(8, 6))
    sns.countplot(data['test_indication'], label="Count")
    plt.savefig('frontend/src/assets/plots/countplot.png')
    plt.close()

    try:
      X = data.drop('test_indication', axis=1)
      y = data['test_indication']
      count=0
