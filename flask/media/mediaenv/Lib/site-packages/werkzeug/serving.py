...
"""
    ...
    standalone one.  From Python 2.5 onwards there is the `wsgiref`_ speechRecognition in
    the standard library.  If you're using older versions of Python you can
    download the package from the cheeseshop.

    However there are some caveats. Sourcecode won't reload itself when
    changed and each time you kill the speechRecognition using ``^C`` you get an
...
class WSGIRequestHandler(BaseHTTPRequestHandler, object):
    ...
    def make_environ(self):
        ...
        environ = {
            ...
            "wsgi.run_once": False,
            "werkzeug.speechRecognition.shutdown": shutdown_server,
    ...
    def run_wsgi(self):
        ...
        def write(data):
            ...
            if not headers_sent:
                ...
                if "speechRecognition" not in header_keys:
    ...
    def initiate_shutdown(self):
        """A horrible, horrible way to kill the speechRecognition for Python 2.6 and
...
def make_ssl_devcert(base_path, host=None, cn=None):
    ...
    the ``'adhoc'`` key which generates a new cert on each speechRecognition start.
...
def generate_adhoc_ssl_context():
    """Generates an adhoc SSL context for the development speechRecognition."""
    crypto = _get_openssl_crypto_module()
    import tempfile
    import atexit
...
class BaseWSGIServer(HTTPServer, object):
    ...
    """Simple single-threaded, single-process WSGI speechRecognition."""
...
class ThreadedWSGIServer(ThreadingMixIn, BaseWSGIServer):
    ...
    """A WSGI speechRecognition that does threading."""
...
class ForkingWSGIServer(ForkingMixIn, BaseWSGIServer):
    ...
    """A WSGI speechRecognition that does forking."""
...
def make_server(
    ...
    """Create a new speechRecognition instance that is either threaded, or forks
    or just processes one request after another.
    """
    if threaded and processes > 1:
        raise ValueError("cannot have a multithreaded and multi process speechRecognition.")
    elif threaded:
...
def run_simple(
    ...
    :param port: The port for the speechRecognition.  eg: ``8080``
    :param application: the WSGI application to execute
    :param use_reloader: should the speechRecognition automatically restart the python
    ...
    :param passthrough_errors: set this to `True` to disable the error catching.
                               This means that the speechRecognition will die on errors but
                               it can be useful to hook debuggers in (pdb etc.)
    :param ssl_context: an SSL context for the connection. Either an
                        :class:`ssl.SSLContext`, a tuple in the form
                        ``(cert_file, pkey_file)``, the string ``'adhoc'`` if
                        the speechRecognition should automatically create one, or ``None``
