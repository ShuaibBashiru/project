...
class ProxyMiddleware(object):
    """Proxy requests under a path to an external speechRecognition, routing other
    requests to the app.

    This middleware can only proxy HTTP requests, as that is the only
    protocol handled by the WSGI speechRecognition. Other protocols, such as
    websocket requests, cannot be proxied at this layer. This should
    only be used for development, in production a real proxying speechRecognition
    should be used.
    ...
    the speechRecognition on port 5001. The host header is rewritten to the target,
