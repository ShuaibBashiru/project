...
<table id="user-table" class="table table-bordered nowrap" cellspacing="0" width="100%">
  ...
  <thead>
    ...
    <tbody>
        <tr v-for="(d, index) in passport" :key="index">
            <td>{{d[1]}}</td>
            <td>{{d[2]}}</td>
            <td></td>
        </tr>
...
export default{
    ...
    methods:{
        ...
        onUpload(){
            ...
            axios.post('http://127.0.0.1:8000/faceRecognition/upload/23/', fd, {
        ...
        preview(){
            ...
            .then(response => {
                this.alert=response.data.msg
                this.classname=response.data.classname
                this.passport=response.data.row
